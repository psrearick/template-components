...
import {bodyScroll, createElement, ElementGenerator, makeId} from "./utilities";
...
export default class PageBuilder {
  ...
  getBuildList = () => {
    ...
    return Array.from(entryElements).map(element => element.getAttribute('data-component'));
  };
  ...
  setPreviewWindowHTML = async () => {
    ...
    this.previewWindowHTML = await this.generator.fetchCode(new URL('../Templates/PreviewWindow.html', import.meta.url));
  };
  ...
  registerPreview = () =>
    document.querySelector('#preview-build')
 ...
 registerResize = () =>
  ...
  getHTMLForComponent = (name) => {
    ...
    const replacedCode = this.generator.replaceProps(component.html.original, props);
  ...
  getJSForComponent = (name) => {
    ...
    const replacedCode = this.generator.replaceProps(component.js.original, props);
  ...
  constructDocument = () => {
    ...
    (new ElementGenerator('div', doc)).setContent(this.pageCode.pageHTML.join('')).append('body');


    const links = document.querySelectorAll('link');
    Array.from(links).forEach(link =>
      (new ElementGenerator('link', doc))
        .setAttributes({href: link.href, rel: link.rel}).append('head')
    );

    const styles = document.querySelectorAll('style');
    Array.from(styles).forEach(style =>
      (new ElementGenerator('style', doc)).setContent(style.innerHTML).append('head')
    );
    ...
    (new ElementGenerator('meta', doc)).setAttributes({
      name: "viewport",
    ...
    (new ElementGenerator('script', doc)).setContent(jsString).appendToElement(this.frame.contentDocument.body);
  };
